...
#define g_assert_not_reached()		G_STMT_START{		      \
     kprintf ("file %s: line %d (%s): \"should not be reached\"",     \
	      __FILE__,						      \
	      __LINE__,						      \
...
#define g_assert(expr)			G_STMT_START{\
     if (!(expr))				     \
       kprintf ("file %s: line %d: \"%s\"",	     \
		__FILE__,			     \
		__LINE__,			     \
		#expr);			}G_STMT_END

#define g_assert_not_reached()		G_STMT_START{		      \
     kprintf ("file %s: line %d: \"should not be reached\"",	      \
	      __FILE__,						      \
	      __LINE__);		}G_STMT_END
...
#define g_return_if_fail(expr)		G_STMT_START{		  \
     if (!(expr))						  \
       {							  \
	 kprintf ("file %s: line %d: assertion. \"%s\" failed", \
		    __FILE__,					  \
		    __LINE__,					  \
		    #expr);					  \
	 return;						  \
...
void g_message (const char *format, ...) G_GNUC_PRINTF (1, 2);
void g_print   (const char *format, ...) G_GNUC_PRINTF (1, 2);
