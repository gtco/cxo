...
g_malloc (gulong size)
{
  ...
  if (!p)
    kprintf("could not allocate %d bytes", size);
...
g_malloc0 (gulong size)
{
  ...
  p = NULL /* @todo implement calloc *** (gpointer) calloc (size, 1)*/;
  if (!p)
    kprintf ("g_malloc0 : could not allocate %d bytes", size);
	   ...
	   gulong   size)
{
  ...
  if (!mem)
  ...
  else
    {
      ...
      t = (gulong*) ((guchar*) mem - SIZEOF_LONG);
      if (*t >= 1)
		kprintf ("trying to realloc freed memory\n");
      mem = t;
#endif /* ENABLE_MEM_CHECK */

      p = NULL /* @todo implement realloc *** (gpointer) realloc (mem, size)*/;
    }

  if (!p)
    kprintf ("g_realloc : could not reallocate %d bytes", size);
  ...
  if (mem)
    {
      ...
      t = (gulong*) ((guchar*) mem - SIZEOF_LONG);
      if (*t >= 1)
		kprintf ("freeing previously freed memory\n");
      *t += 1;
      mem = t;

      _memset ((guchar*) mem + 8, 0, size);
...
g_mem_profile (void)
{
  ...
  for (i = 0; i < 4095; i++)
    if (allocations[i] > 0)
      kprintf ("%u allocations of %d bytes\n", allocations[i], i + 1);

  if (allocations[4095] > 0)
    kprintf ("%u allocations of greater than 4095 bytes\n", allocations[4095]);
  kprintf ("%u bytes allocated\n", allocated_mem);
  kprintf ("%u bytes freed\n", freed_mem);
  kprintf ("%u bytes in use\n", allocated_mem - freed_mem);
#endif /* ENABLE_MEM_PROFILE */
}
...
g_mem_check (gpointer mem)
{
  ...
  if (*t >= 1)
    kprintf ("mem: 0x0%x has been freed: %u\n", (gulong) mem, *t);
#endif /* ENABLE_MEM_CHECK */
}
